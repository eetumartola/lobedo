...
- **D4**: Consolidate node registry (reduce remaining manual duplication) (done)
- **D5**: Shared scene types crate used by core/render/app (done)
- **D6**: Graph adjacency/index for input link lookup (done)
- **D7**: Texture decode cache (path + mtime / URL revision) (done)
- **D8**: Add core tests (graph/eval/migrations/registry) (done)
...
- **T3**: Parallelize splat ops (Prune, Regularize, LOD/Decimate) (done)
- **T4**: Parallelize mesh ops (Delete/Group, Copy to Points/Transform, Ray) (done)
- **T5**: Parallelize per-primitive evaluation inside Geometry nodes (mesh/splat lists) (done)
...
- **W1**: Add Volume primitive type + core representation (done)
