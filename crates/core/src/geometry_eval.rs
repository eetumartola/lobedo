    ...
    let report = evaluate_from_with_progress(
        ...
        |node_id, params| {
            ...
            let kind = if node.kind_id.is_empty() {
                ...
                builtin_kind_from_id(&node.kind_id).or_else(|| builtin_kind_from_name(&node.name))
            }
            .ok_or_else(|| {
                ...
                match pin.pin_type {
                    ...
                    _ => {
                        let geometry = if let Some(link) = link {
                            ...
                            let geometry = outputs.get(&upstream_id).ok_or_else(|| {
                                format!("missing upstream output {upstream_id:?}")
                            })?;
                            Some(geometry.clone())
                        } else {
                            None
